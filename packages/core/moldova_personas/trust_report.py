...
# Estimated-only fields that should gate HIGH_STAKES use cases.
ESTIMATED_HIGH_STAKES_FIELDS: Set[str] = {
    "occupation",
    "occupation_sector",
}
...
USE_CASE_THRESHOLDS = {
    ...
    UseCaseProfile.HIGH_STAKES: {
        ...
        "max_fallback_ratio_critical": 0.40,
        "max_fallback_ratio_super_critical": 0.50,  # Allow official census fallback where PxWeb is missing
...
class TrustReport:
    ...
    estimated_fields_high_stakes: List[str] = field(default_factory=list)
    """Estimated fields that should gate HIGH_STAKES use cases"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "estimated_fields_high_stakes": self.estimated_fields_high_stakes,
...
class TrustReportGenerator:
    ...
    def generate_report(
        ...
        if provenance_info:
            ...
            report.estimated_fields_high_stakes = [
                f for f in report.estimated_fields
                if f in ESTIMATED_HIGH_STAKES_FIELDS
            ]
    ...
    def _calculate_marginal_errors(
        ...
        # Education (adult-only target)
        try:
            add_error("education", self.census.ADULT_EDUCATION_DISTRIBUTION, lambda p: p.education_level)
        except Exception:
            pass

        # Marital status (adult-only target)
        try:
            add_error("marital_status", self.census.ADULT_MARITAL_STATUS_DISTRIBUTION, lambda p: p.marital_status)
        except Exception:
            pass
        ...
        # Employment status (adult-only target)
        try:
            add_error("employment_status", self.census.ADULT_EMPLOYMENT_STATUS_DISTRIBUTION, lambda p: p.employment_status)
        except Exception:
            pass
    ...
    def _apply_gates(self, report: TrustReport) -> Tuple[bool, List[str]]:
        ...
        if report.estimated_fields:
            if self.use_case == UseCaseProfile.HIGH_STAKES:
                critical_estimated = report.estimated_fields_critical + report.estimated_fields_high_stakes
                if critical_estimated:
                    reasons.append(
                        f"Estimated field used: {', '.join(sorted(set(critical_estimated)))}"
                    )
                    hard_gate = True
