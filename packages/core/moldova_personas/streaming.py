...
import json
import logging
from typing import Iterator, Optional, Callable, Dict, Any, Iterable, Set
...
class StreamingJSONLExporter:
    ...
    def __init__(self, filepath: str, drop_fields: Optional[Iterable[str]] = None):
        """
    ...
    def write_batch(self, personas: list):
        ...
        for persona in personas:
            line = json.dumps(persona.model_dump(exclude=self.drop_fields), ensure_ascii=False)
...
class StreamingCSVExporter:
    ...
    def __init__(
        ...
        Args:
            ...
            drop_fields: Optional fields to exclude
        """
        self.filepath = Path(filepath)
    ...
    def _row(self, persona: Persona) -> Dict[str, Any]:
        data = persona.model_dump(exclude=self.drop_fields)
        for col in ["skills_and_expertise_list", "hobbies_and_interests_list"]:
            if col in data:
                data[col] = "|".join(data[col]) if data[col] else ""
        return data
    ...
    def write_batch(self, personas: list):
        ...
        if not personas:
            return
        if self.writer is None:
            first_row = self._row(personas[0])
            ...
            start_index = 0
        for persona in personas[start_index:]:
            self.writer.writerow(self._row(persona))
        self.total_rows += len(personas)
    ...
    def close(self):
        if self.file_handle:
            self.file_handle.close()
            self.file_handle = None
            logger.info(f"CSV export complete: {self.total_rows} rows to {self.filepath}")
...
def generate_and_export_streaming(
    ...
    Args:
        ...
        batch_size: Number of personas per batch
        seed: Random seed
        format: Output format ("parquet", "jsonl", or "csv")
        progress_callback: Optional callback(complete, total) for progress updates
        drop_fields: Optional fields to exclude from output
    ...
    if format == "parquet":
        ...
        exporter = StreamingJSONLExporter(output_path, drop_fields=drop_fields)
    elif format == "csv":
        exporter = StreamingCSVExporter(
            output_path, separator=separator, drop_fields=drop_fields
        )
    else:
        raise ValueError(f"Unsupported format: {format}")

    with exporter as exporter:
