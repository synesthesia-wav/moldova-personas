...
from .paths import config_dir, data_path
...
def load_districts_by_region() -> Dict[str, list]:
    ...
    path = config_dir() / "districts_by_region.json"
    if not path.exists():
        return {}
...
def get_district_distribution(strict: Optional[bool] = None) -> Optional[Dict[str, float]]:
    ...
    candidates = [
        data_path("nbs_population_by_district_2024.json"),
        config_dir() / "district_distribution_2024.json",
    ]
