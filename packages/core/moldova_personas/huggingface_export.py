"""
Export personas to HuggingFace Datasets format.

Compatible with the reference public schema for easy integration
...
class HuggingFaceExporter:
    ...
    # Reference schema-compatible field order
    FIELD_ORDER = [
        "uuid",
    ...
    def to_jsonl(self, personas: List[Union[Persona, OCEANPersona, Dict[str, Any]]], output_path: Path) -> Path:
        """
    ...
    def to_json(self, personas: List[Union[Persona, OCEANPersona, Dict[str, Any]]], output_path: Path) -> Path:
    ...
    def to_csv(self, personas: List[Union[Persona, OCEANPersona, Dict[str, Any]]], output_path: Path) -> Path:
    ...
    def to_parquet(self, personas: List[Union[Persona, OCEANPersona, Dict[str, Any]]], output_path: Path) -> Path:
        """
    ...
    def generate_dataset_card(
        ...
        Follows the standard dataset card style.
        """
        card = f"""---
language:
  - ro
...
dataset = load_dataset("your-org/moldova-personas")
...
```bibtex
@dataset{{moldova_personas_{version},
  title = {{Moldova-Personas: Synthetic Personas Grounded in Census Data}},
  year = {{2026}},
...
"""
    ...
    def export_all(
        self,
        personas: List[Union[Persona, OCEANPersona, Dict[str, Any]]],
        output_dir: Path,
    ...
    def _to_ordered_dict(self, persona: Union[Persona, OCEANPersona, Dict[str, Any]]) -> Dict[str, Any]:
        """Convert persona to ordered dict following reference schema."""
        ...
        # Map Moldova Persona fields to reference schema
        if "municipality" not in data and "district" in data:
            data["municipality"] = data.get("district")
        if "state" not in data and "region" in data:
            data["state"] = data.get("region")
        if "locality" not in data and "city" in data:
            data["locality"] = data.get("city")

        # Ensure list fields are serialized consistently
        for field in ["skills_and_expertise_list", "hobbies_and_interests_list"]:
