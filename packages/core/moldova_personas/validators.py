...
class ValidationPipeline:
    ...
    def _validate_structural(self, persona: Persona) -> Tuple[List[ValidationError], List[ValidationError]]:
        ...
        # Age validation
        if not (AgeConstraints.MIN_PERSONA_AGE <= persona.age <= AgeConstraints.MAX_REALISTIC_AGE):
            errors.append(ValidationError(
                field="age",
                message=(
                    f"Age {persona.age} out of valid range "
                    f"[{AgeConstraints.MIN_PERSONA_AGE}, {AgeConstraints.MAX_REALISTIC_AGE}]"
                ),
                severity="error",
    ...
    def _validate_statistical(self, personas: List[Persona], 
        ...
        """
        try:
            return self._validate_statistical_vectorized(personas, tolerance)
        except Exception:
            return self._validate_statistical_python(personas, tolerance)

    def _validate_statistical_python(self, personas: List[Persona],
                                    tolerance: float) -> Tuple[List[ValidationError], List[ValidationError]]:
        """Pure-Python statistical validation (fallback)."""
        errors = []
        warnings = []
        ...
        # Check education distribution (adult-only target)
        ed_counts = Counter(p.education_level for p in personas)
        ed_dist = {k: v / n for k, v in ed_counts.items()}
        for education, target in self.census.ADULT_EDUCATION_DISTRIBUTION.items():
    ...
    def _validate_statistical_vectorized(self, personas: List[Persona],
                                        tolerance: float) -> Tuple[List[ValidationError], List[ValidationError]]:
        """Vectorized statistical validation using pandas."""
        ...
        df = pd.DataFrame({
            "sex": [p.sex for p in personas],
            "region": [p.region for p in personas],
            "ethnicity": [p.ethnicity for p in personas],
            "residence_type": [p.residence_type for p in personas],
            "education_level": [p.education_level for p in personas],
        })

        def dist(series: pd.Series) -> Dict[str, float]:
            return series.value_counts(normalize=True).to_dict()

        sex_dist = dist(df["sex"])
        for sex, target in self.census.SEX_DISTRIBUTION.items():
            actual = sex_dist.get(sex, 0.0)
            if abs(actual - target) > tolerance:
                warnings.append(ValidationError(
                    field="sex_distribution",
                    message=f"Sex '{sex}': target {target:.3f}, actual {actual:.3f} (diff: {abs(actual-target):.3f})",
        ...
        if strict_geo_enabled():
            ...
            if derived:
                target_region_dist = derived
        for region, target in target_region_dist.items():
            actual = region_dist.get(region, 0.0)
            if abs(actual - target) > tolerance:
                warnings.append(ValidationError(
                    field="region_distribution",
                    message=f"Region '{region}': target {target:.3f}, actual {actual:.3f} (diff: {abs(actual-target):.3f})",
        ...
        ethnicity_dist = dist(df["ethnicity"])
        for ethnicity, target in self.census.ETHNICITY_DISTRIBUTION.items():
            actual = ethnicity_dist.get(ethnicity, 0.0)
            if abs(actual - target) > tolerance * 2:
                warnings.append(ValidationError(
                    field="ethnicity_distribution",
                    message=f"Ethnicity '{ethnicity}': target {target:.3f}, actual {actual:.3f} (diff: {abs(actual-target):.3f})",
                    severity="warning"
        ...
        residence_dist = dist(df["residence_type"])
        for residence, target in self.census.RESIDENCE_TYPE_DISTRIBUTION.items():
            actual = residence_dist.get(residence, 0.0)
            if abs(actual - target) > tolerance:
                warnings.append(ValidationError(
                    field="residence_distribution",
                    message=f"Residence '{residence}': target {target:.3f}, actual {actual:.3f} (diff: {abs(actual-target):.3f})",
        ...
        education_dist = dist(df["education_level"])
        for education, target in self.census.ADULT_EDUCATION_DISTRIBUTION.items():
            actual = education_dist.get(education, 0.0)
            if abs(actual - target) > tolerance * 1.5:
                warnings.append(ValidationError(
                    field="education_distribution",
                    message=f"Education '{education}': target {target:.3f}, actual {actual:.3f} (diff: {abs(actual-target):.3f})",
