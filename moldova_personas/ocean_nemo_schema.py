"""
NeMo-style OCEAN schema with t_score, label, description.

Matches Nemotron-Brazil's structured personality representation.
"""
...
class OCEANTrait:
    """
    Single OCEAN trait in NeMo format.
...
@dataclass
class OCEANProfileNeMo:
    """
    Complete OCEAN profile in NeMo schema format.
...
def convert_to_nemo_schema(raw_scores: Dict[str, int]) -> Dict[str, any]:
    """
    Convert raw OCEAN scores to NeMo schema.
    ...
    Returns:
        Dict in NeMo format with t_score, label, description for each trait
    """
    profile = OCEANProfileNeMo.from_raw_scores(
