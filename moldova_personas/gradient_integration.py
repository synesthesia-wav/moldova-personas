    ...
    if generate_narratives and profile != UseCaseProfile.ANALYSIS_ONLY:
        ...
        try:
            ...
            nar_gen = NarrativeGenerator(
                ...
                api_key=llm_api_key,
                model=llm_model,
                allow_mock_narratives=config.allow_mock_narratives if config.allow_mock_narratives is not None else True,
            )
            delay = 0.0
