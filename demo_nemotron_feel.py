#!/usr/bin/env python3
"""
Demo of Nemotron-feel patches applied to persona generation.
...
Usage:
  python demo_nemotron_feel.py --count 3 --provider dashscope --api-key <key>
"""
...
from moldova_personas.nemotron_feel_patch import (
    generate_behavioral_cues,
    generate_realism_constraints,
    select_anchors,
    generate_stage_b_prompt_nemotron_feel,
    generate_stage_c_prompt_nemotron_feel,
    NemotronFeelValidator,
    OPENING_MOVES,
    validate_trait_leak,
...
async def generate_one_persona_nemotron_feel(
    base_persona,
    name: str,
    llm_client,
    validator: NemotronFeelValidator
) -> dict:
    """Generate one persona with Nemotron-feel patches."""
    ...
    stage_b_prompt = generate_stage_b_prompt_nemotron_feel(
    ...
    stage_c_prompt = generate_stage_c_prompt_nemotron_feel(
...
async def main():
    parser = argparse.ArgumentParser(description="Nemotron-feel demo")
    parser.add_argument("--count", "-n", type=int, default=3)
    parser.add_argument("--provider", default="mock")
    parser.add_argument("--api-key", default=None)
    parser.add_argument("--output", "-o", default="nemotron_feel_output.json")
    ...
    # Generate
    print(f"Generating {args.count} personas with Nemotron-feel patches...\n")
    ...
    for i, (base, name) in enumerate(zip(base_personas, names), 1):
        ...
        if args.provider == "mock":
            ...
            result = await generate_one_persona_nemotron_feel(
    ...
    output = {
        "metadata": {
            "format": "Nemotron-Moldova-Feel-Patched",
            "count": len(results),
            "features": [
                "implicit_ocean",
                "varied_openings", 
