#!/usr/bin/env python3
"""
Full Nemotron-parity demo with:
1. NeMo-style OCEAN schema (t_score, label, description)
2. Population-level QA (stratified checks, divergence metrics)
3. Complete two-layer generation

Usage:
  python demo_nemo_full.py --count 50 --provider dashscope --api-key <key>
"""
...
from moldova_personas.ocean_nemo_schema import OCEANProfileNeMo, convert_to_nemo_schema
from moldova_personas.population_qa import PopulationQA, quick_population_qa


# Simplified NeMo-style persona with proper OCEAN schema
class NemotronPersonaNeMo:
    """Nemotron persona with NeMo-style OCEAN schema."""
    
    def __init__(self, base_data: Dict, ocean_nemo: OCEANProfileNeMo):
        self.data = base_data
        self.ocean = ocean_nemo
    
    def to_dict(self) -> Dict[str, Any]:
        """Export to dictionary with NeMo OCEAN format."""
        result = self.data.copy()
        # Replace raw OCEAN scores with NeMo schema
...
async def generate_nemo_batch(
    count: int,
    provider: str = "mock",
    api_key: Optional[str] = None
) -> List[NemotronPersonaNeMo]:
    """Generate batch with NeMo-style OCEAN."""
    
    print(f"Generating {count} personas with NeMo-style OCEAN...")
    ...
    for p in base_personas:
        ...
        # Convert to NeMo schema
        ocean_nemo = OCEANProfileNeMo.from_raw_scores(
        ...
        results.append(NemotronPersonaNeMo(base_data, ocean_nemo))
...
def run_population_qa(personas: List[NemotronPersonaNeMo]) -> Dict[str, Any]:
...
def export_nemo_format(
    personas: List[NemotronPersonaNeMo],
    output_dir: Path,
    qa_report: Dict[str, Any]
):
    """Export in NeMo/Nemotron format."""
    
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # JSONL (primary format)
    jsonl_path = output_dir / "nemotron_moldova_nemo.jsonl"
    with open(jsonl_path, 'w', encoding='utf-8') as f:
        for p in personas:
            f.write(json.dumps(p.to_dict(), ensure_ascii=False) + '\n')
    
    # Full JSON with metadata
    json_path = output_dir / "nemotron_moldova_full.json"
    full_data = {
        "metadata": {
            "format": "Nemotron-Moldova-NeMo",
            "version": "2.0.0",
            "count": len(personas),
            "schema": {
                "ocean": "NeMo (t_score, label, description)",
...
{len(personas):,} synthetic Moldovan personas with NeMo-style OCEAN Big Five schema.
...
async def main():
    parser = argparse.ArgumentParser(description="Full Nemotron-parity demo")
    parser.add_argument("--count", "-n", type=int, default=50)
    parser.add_argument("--provider", default="mock")
    parser.add_argument("--api-key", default=None)
    parser.add_argument("--output-dir", default="output_nemo_full")
    
    args = parser.parse_args()
    
    print("=" * 70)
    print("NEMOTRON-PARITY: NeMo SCHEMA + POPULATION QA")
    print("=" * 70)
    print(f"\nGenerating {args.count} personas...")
    
    # Generate
    personas = await generate_nemo_batch(
        count=args.count,
    ...
    # Show sample
    print("\n" + "-" * 70)
    print("SAMPLE PERSONA (NeMo OCEAN Schema):")
    print("-" * 70)
    ...
    paths = export_nemo_format(personas, output_dir, qa_report)
