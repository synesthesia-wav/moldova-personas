#!/usr/bin/env python3
"""
Scale test: Generate 100 OCEAN pipeline personas.
...
Usage:
  python tools/scripts/scale_test_100.py --provider dashscope --api-key <key>
  python tools/scripts/scale_test_100.py --mock  # Fast test without LLM
"""
...
from moldova_personas.ocean_pipeline import generate_ocean_personas
...
async def run_scale_test(
    ...
    print("=" * 70)
    print(f"SCALE TEST: {count} OCEAN PIPELINE PERSONAS")
    print("=" * 70)
    ...
    # Stage 2: Generate OCEAN personas
    print("Stage 2: Running OCEAN pipeline (3 stages + validation)...")
    stage_start = time.time()
    
    metrics.start()
    
    try:
        results = await generate_ocean_personas(
            ...
            # Soft validation handled separately
        )
        
        # Extract personas from results
        ocean_personas = list(results)
        metrics.successful = len(ocean_personas)
    ...
    stage_duration = time.time() - stage_start
    metrics.record_stage("ocean_pipeline", stage_duration)
    print(f"  ✓ Generated {len(ocean_personas)} personas in {stage_duration:.1f}s")
    print()
    ...
    if provider == "dashscope":
        ...
        tokens_total = estimated_tokens_per_persona * len(ocean_personas)
    ...
    # Save full dataset
    output_file = output_path / f"ocean_personas_{count}.json"
    dataset = {
        "metadata": {
            "format": "Moldova-Personas-OCEAN",
    ...
    try:
        import csv
        csv_file = output_path / f"ocean_personas_{count}.csv"
        if ocean_personas:
            with open(csv_file, 'w', newline='', encoding='utf-8') as f:
                writer = csv.DictWriter(f, fieldnames=ocean_personas[0].to_dict().keys())
                writer.writeheader()
                for p in ocean_personas:
    ...
    # Sample validation
    if validate_sample > 0 and ocean_personas and provider != "mock":
        ...
        sample_indices = list(range(0, min(validate_sample, len(ocean_personas))))
        validation_results = []
        
        for i in sample_indices:
            try:
                results = await validators.validate_full(ocean_personas[i])
...
def main():
    parser = argparse.ArgumentParser(description="Scale test for OCEAN pipeline")
